...
def gamma_plus_right_e0(epsilon_zero):
    return 1  # 2 * param.gamma_zero_right * np.exp(param.tunneling_exponent_right * epsilon_zero) * (1 - fermi_distribution(epsilon_zero))


def gamma_minus_right_e0(epsilon_zero):
    return 1  # param.gamma_zero_right * np.exp(param.tunneling_exponent_right * epsilon_zero) * (fermi_distribution(epsilon_zero))
...
# Tunneling rates
def gamma_plus_right_modified(w, e_bias, epsilon_zero, width):
    return 2 * param.gamma_zero_right * np.exp(- param.tunneling_exponent_right * energy_change_plus_right(w, e_bias)) * (1 - fermi_distribution(- energy_change_plus_right(w, e_bias))) * (gaussian(energy_change_minus_right(w, e_bias), epsilon_zero, width) / gamma_plus_right_e0(epsilon_zero))


def gamma_minus_right_modified(w, e_bias, epsilon_zero, width):
    return param.gamma_zero_right * np.exp(param.tunneling_exponent_right * energy_change_minus_right(w, e_bias)) * (fermi_distribution(energy_change_minus_right(w, e_bias))) * (gaussian(energy_change_minus_right(w, e_bias), epsilon_zero, width) / gamma_minus_right_e0(epsilon_zero))
...
def d_gamma_plus_right_modified(w, e_bias, epsilon_zero, width):
    return ((2 * param.gamma_zero_right) / (width ** 2)) * np.exp((param.tunneling_exponent_right + (1 / param.kT)) * energy_change_minus_right(w, e_bias)) * (gaussian(energy_change_minus_right(w, e_bias), epsilon_zero, width) / gamma_plus_right_e0(epsilon_zero)) \
           * (- epsilon_zero + energy_change_minus_right(w, e_bias) + (param.tunneling_exponent_right + (1 / param.kT)) * (width ** 2) + (- epsilon_zero + energy_change_minus_right(w, e_bias) + param.tunneling_exponent_right * (width ** 2))) \
           * (fermi_distribution(energy_change_minus_right(w, e_bias)) ** 2)


def d_gamma_minus_right_modified(w, e_bias, epsilon_zero, width):
    return param.gamma_zero_right * np.exp(param.tunneling_exponent_right * energy_change_minus_right(w, e_bias)) * (gaussian(energy_change_minus_right(w, e_bias), epsilon_zero, width) / gamma_minus_right_e0(epsilon_zero)) * (- (1 / param.kT) * np.exp(energy_change_minus_right(w, e_bias) / param.kT)
            + (1 + np.exp(energy_change_minus_right(w, e_bias) / param.kT)) * (param.tunneling_exponent_right + ((- epsilon_zero + energy_change_minus_right(w, e_bias)) / (width ** 2)))) \
            * (fermi_distribution(energy_change_minus_right(w, e_bias)) ** 2)
...
def gamma_plus(w, e_bias, epsilon_zero, width):
    return gamma_plus_left(w, e_bias) + gamma_plus_right_modified(w, e_bias, epsilon_zero, width)


def gamma_minus(w, e_bias, epsilon_zero, width):
    return gamma_minus_left(w, e_bias) + gamma_minus_right_modified(w, e_bias, epsilon_zero, width)


def d_gamma_plus(w, e_bias, epsilon_zero, width):
    return d_gamma_plus_left(w, e_bias) + d_gamma_plus_right_modified(w, e_bias, epsilon_zero, width)


def d_gamma_minus(w, e_bias, epsilon_zero, width):
    return d_gamma_minus_left(w, e_bias) + d_gamma_minus_right_modified(w, e_bias, epsilon_zero, width)
...
def plot_kappa_tilde_region_iii_symmetric():
    ...
    E = count * d_energy
    filename = "RegioniiiKappaDensity " + f"Emax={E}Wc-symmetric.png"
    plt.savefig("../out/" + filename, bbox_inches='tight')
    print("Saved: " + filename)
...
plot_kappa_tilde_region_iii_symmetric()
