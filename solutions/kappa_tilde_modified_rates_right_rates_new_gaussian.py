...
# Tunneling rates
def gamma_plus_right_modified(w, e_bias, epsilon_zero, width, average=True):
    return 2 * param.gamma_zero_right * np.exp(- param.tunneling_exponent_right * energy_change_plus_right(w, e_bias, average)) * (1 - fermi_distribution(- energy_change_plus_right(w, e_bias, average))) * (gaussian(energy_change_minus_right(w, e_bias, average), epsilon_zero, width) / gamma_plus_right_e0(epsilon_zero))


def gamma_minus_right_modified(w, e_bias, epsilon_zero, width, average=True):
    return param.gamma_zero_right * np.exp(param.tunneling_exponent_right * energy_change_minus_right(w, e_bias, average)) * (fermi_distribution(energy_change_minus_right(w, e_bias, average))) * (gaussian(energy_change_minus_right(w, e_bias, average), epsilon_zero, width) / gamma_minus_right_e0(epsilon_zero))
...
def plot_plus_rates_bias_slice(e_bias):
    ...
    for i in range(len(w_list)):
        ...
        gamma_right[i] = gamma_plus_right_modified(w_list[i], e_bias, epsilon_zero, width, False)
...
def plot_minus_rates_bias_slice(e_bias):
    ...
    for i in range(len(w_list)):
        ...
        gamma_right[i] = gamma_minus_right_modified(w_list[i], e_bias, epsilon_zero, width, False)
